...
int row,column,rowsize,columnsize,i,errtol,matsize,index;
bool test;
float smin,smax,ds,r,mid,ratio,left,right,up,down;
BMP Image;
cout<<"Done."<<endl;
...
while(test == true)
  ...
  for(row=0;row<rowsize;row++)
  {
	...
	vals[row][column][-(i%2)+1] = (up+down+right+left)/4.00;
      ...
      if(ratio <= (1-errtol/100.0) || ratio >= (1+errtol/100.0))
      {
        test = true;
      }          
    }
...
for(row=0;row<rowsize;row++) {
  for(column=0;column<columnsize;column++) {
    ...
    if(index==0){
      ...
      datafile<<column<<" "<<rowsize-row<<" "<<vals[row][column][((i%2)==0)?0:1]<<"\n";
    }
    else if(index==2){
